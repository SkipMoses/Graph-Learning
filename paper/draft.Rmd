...
In order to generalize our model, we will take inspiration from the ideas used to generalize linear regression to logistic regression. Let $p(x = 1)$ be the probability a signal value of $1$ is observed on node $x$. Our model will be of the form 
\begin{align}
\text{logit}(p) = \chi h + p(x = 1) + \epsilon
\end{align}

Where $\chi$ is our matrix of eigenvalues for our graph. Recall, in binomial distribution the expectation is exactly $p(x = 1)$. 
